...
def login():
    ...
    cmd.execute(
        "select * from login where username='" + str(name) + "' and password='" + str(pword) + "' and type!='pending' ")
    s = cmd.fetchone()
    print(s)
    if s == None:
        ...
        if s is None:
            ...
            if s is None:
                ...
                if s is None:
                    ...
                    cmd.execute("insert into user_reg  values(null,'" + str(
                        id) + "','" + fname + "','" + mname + "','" + lname + "','" + phno + "','" + email + "'")
                    con.commit()
                    return jsonify({'task': "success"})
                else:
                    return jsonify({'task': "Email Exists"})
...
def addnewturf():
    try:
        ...
        cmd.execute("insert into turf_registration  values(null,'" + str(
            id) + "','" + name + "','" + place + "','" + landmark + "','" + phno + "','" + email + "','" + fn + "','" + latt + "','" + long + "')")
        id=con.insert_id()
        cmd.execute("INSERT INTO `turfrating` VALUES(NULL,'"+id+"',0,5,'nn')")
        con.commit()
        return jsonify({'task': "success"})
    except Exception as e:
        print(str(e))
...
def bookinghistory():
    ...
    print(tid)
    cmd.execute(
        "SELECT `booking`.*,`turf_registration`.* ,`user_reg`.`fname`,`lname`,`mname` FROM `booking` JOIN `turf_registration` ON `booking`.`tid`=`turf_registration`.`lid` JOIN `user_reg` ON `user_reg`.`lid`=`booking`.`uid` WHERE `booking`.`tid`='" + tid + "' AND `booking`.`status`='pending'")
    row_headers = [x[0] for x in cmd.description]
    results = cmd.fetchall()
    json_data = []
    for result in results:
...
def incomeview():
    ...
    cmd.execute(
        "SELECT SUM(amount),turf_registration.`name` FROM `payment`  JOIN `turf_registration` ON `turf_registration`.`tid`=`payment`. tid AND `turf_registration`.lid='" + str(
            logid) + "'  WHERE MONTH(DATE)='" + str(month) + "' AND YEAR(DATE)='" + str(
            year) + "'   GROUP BY `payment`.tid ")
